...
are spatially resolved maps of light intensity at specific wavelenghts as seen from a specific relative position.
This fictitious images are further elaborated with pymcfost, whose function has been discussed in chapter 3, to simulate the limited resolution of
...
I then simulated the radiative transfer.
...
The entire dataset is, in fact, stored in a public git-hub repository exploting
...
the index files can be downloaded. They can then be read to obtain a pandas dataframe which containing
a reference to each image and all the parameters stored in its reader. 
It is thus possible to analyze this data to select, in case, a specific subset of the entire database.
The keys of the images filtered in this way can thus be used to download the corresponding images without the need
...
To populate and manage the database, I developed a python package \lstinline{dblib}
...
Three main functions are carried out by these modules: preprocessing through pymcfost 
of the images obtained after the \lstinline{MCFOST} simulation, handling the data stored 
in the index files and downloading the database or some parts from the remote server.

The preprocessing is handled by the submodule pymcfost\_process.py which contains the function 
``read\_dirs\_doimages()'' that reads the \lstinline{FITS} files produced by \lstinline{MCFOST},
and some log files containing the parameters of the simulations, all  organized in a specific structure of subdirectories.
These images are then convolved with gaussian beams of given sizes. Furthermore, some additional paramters are added to the header
file to reach the complete list in table ().
This module was thus used to preprocess and standardize the previously computed
simulations that we collected before adding them to the final database.
In order to simplify the use of this tool, I wrote a CLI script called \lstinline{pymc_prepro} which
allows the use of the functions just mentioned through a command line interface.

The second function of these scripts is the creation and handling of the index.
The two submodules index and index\_keys fulfil this purpose. They provide functions
that read the parameters stored in the \lstinline{FITS} file and generate or update the
index files. Furthermore, they allow the access to this index through the function \lstinline{get_data()}
which merges together, from the two different files, all the data required returning it as a python dictionary.
...
the command line specifying the directory where the unindexed \lstinline{FITS} files are stored in addition
to the optional parameters if needed.
...
This functions are interfaced by the cli script \lstinline{get_db}.
...
We have seen, in the previous section, that a small set of all the parameters exhibit different values distributed
across a sufficiently large range. 
In the development of the database structure, efforts had been made to allow an easy expansion with new
data. The previously discussed scripts were also developed with this purpose.

In order to integrate new images the raw material needed is FITS files obtained from a radiative 
transfer simulation done with \lstinline{MCFOST} or a similar tool. If results of the hydrodynamical 
simulations are available, this next step could be additionally run.
...
When all the simulations available are properly grouped in this way, the \lstinline{pymc_prepro} script
...
to build a script adapt for this situation.
...
At this point the database has been expanded. To share the contribution merging it to the public remote database
a pull request can be opened in the hosting repository. 

The current version of this database can be improved under many aspects. The first suggestion
is the addition of simulation exploring ranges of the parameters which do not vary across the images already present.
...
gaps. It might be interesting to include in the dataset also simulations where the gap is not visible both for simulation with 
and without planets. In this way, these images might be used, for example, to train a neural network for detecting the presence of
planets. With this addition it may also be necessary to update the header of these files including some flags which would allow 
the filtering of the data depending on what is needed.

Images of discs with double stars or more than one planet might also be included to include all the possible morphologies.
