...
Since 1995 over 4000 exoplanets and, more recently, hundreds of protoplanetary discs have been observed in an extraordinary 
...
from measures of their radiative emission and analysis of the morphologies revelead.
...
allow a fast computation of unknown properties and can be easily explained with theoretical arguments but
lack in accuracy and precision requiring the use of more advanced tools.
Numerical simulations are today's response to these needs. Sophisticated code, such as phantom and mcfost, 
are able to apply the physical models that describe the dynamic, interaction and radiative emission at a small scale 
to simulate the overall disc evolution and synthetize fictious images which would be observed if the system was real.
These results are thus directly compared in the data space. With this method, the masses of some planets embedded 
in discs' gaps were measured with relatively low uncertainty. Veronesi et al. \cite{dstauv}, for example, determined the
presence of a planet in DS Tau gap with a mass of $3.5 \pm 1 M_{jup}$.
...
required and the dozens of physical properties parametrizing the theoretical models which need to be 
...
to relax some of the assumption needed for running numerical simulations and would also require 
...
the main operation required to handle the database. At this stage we were able to collect 24720 different images 
of protoplanetary discs sgnnificantly exploring * parameters.
...
In the first chapter of  this thesis, I will provide an
introduction on the subject of protoplanetary discs and planet formation which 
are the object of our study. Therefore, in the second chapter, I will review the current 
methods used for the characterisation of protoplannetary discs discussing strengths and limitations
of both analytical expressions and numerical simulations. Machine learning, and more specifically neural networks will 
...
reviewing previous attempts and analyzing the feasibility, the limits and the potential of our approach.
...
Future perspectives about its improvement will also be provided.
The last chapter concludes our work with a proof of concept.
...
The main goal of this thesis was the design of the dataset for machine learning application
which was deployed in its first version and made publicly available. A total of 24720 different
images were produced and collected from the results of two groups of numerical simulations allowing
a significant representation of different gas to dust ratio, ages of the disc, inclinations and masses of the embedded 
planets. Some additional parameters are also explored with less significant distributions.
...
We showed an example of usage for the dataset and explained the steps required to expand its content.
Processes which we believe to be simple and well interfaced with the modern technlogies used in the field
of machine learning.

The results obtained with the proof of concept are promising. The trained model successfully
characterized the planets embedded in protoplanetary discs of the test set images
...
Furthermore, the final application of the trained model to the real observation of DS Tau 
provided an estimation of the planet mass of $3.30 \pm 0.15 M_{jup}$ in agreement with the result,
$3.5 \pm 1 M_{jup}$,
...
directly analyse the observations are the main advantages we highlighted which could
furthermore allow the application of an adequately trained model to hypothetical large surveys
of protoplanetary discs. In a similar scenario, our approach would allow to obtain a fast characterization
...
eye to future developments. First, some parameters characterising the data
did not properly represent the parameter space limiting the possibility of research over these 
images. For example, each of them was obtained probing the simulated emission at $\SI{1.3}{\mu m}$,
observations at different wavelenghts might be useful to inquire how the
performances of neural network algorothms change according to the type
of observations. For the same reason, the inclusion of simulations showing the scattered light and the gas molecular
emission might also be interesting. In addition to that, hydrodynamical parameters such as 
...
simpler and easier to maintain solutions. First, the data should be moved in a databasse such as MongoDB or MySQL
provided with more advanced tools for the management and indexing of large datasets.
Then, it could be stored on an indipendent server developing a code to allow its access filtering the data on 
this server to avoid the user time and memory consuming operations. A system of this type could also be coded to
produce, when requested, new images convolved with beams of specific sizes.
...
could be relevant in the application of a model trained on our dataset to real observations. 
The influence of statistical errors and the background noise shoould thus be studied.
A possible solution consists in using CASA to include all the sources of errors in the dataset images but
the problem could also be attacked from the opposite side developing techniques,
maybe even using machine learning methods, to clean the original observations from all types of errors.
...
model could be implemented improving the choice of the hyperparameters with 
...
Withoout the need for any addition but requiring better performing machines, our 
dataset could be used to study the neural network susceptibility to the image resolution in terms of dimensions of
the beam size used to convolve the original one. 

Different architectures of the neural network could also be explored including, for example, convolutional
...
for istance, a model that returns the planet's mass and the gas to dust ratio.
